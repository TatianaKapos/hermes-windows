 ...
 *
 * Are these all the methods we *need* to say we support Intl? Seems to be
 * missing a few (like ListFormat, Locale, PluralRules, ect.). Matching current
 * Android implentation, first generation, Number, DateTime, Collotor
 *
 * Do we have an internal API or methods that deal with locales? Not sure how to
 * know all the different locales/internalizations? ICU-4C - open source libary,
 * add .dll
 *
 * Do you know anyone who has working on Intl on V8 or Chakra? Wondering if we
 * should reuse some of their code? Referenced their code, Chakra least complete
 *
 * Are there already tests for windows intl or are those specifically for
 * android?
 *
 */
...
// locales - a list of strings for which to get cononical locacle name (ex:
...
// object enables language-sensitive string comparison, initalized with
...
// returns an array containing provided locales that are supported w/o having to
...
new Intl.DateTimeFormat('ko-KR', { dateStyle: 'medium', timeStyle: 'medium'
}).format(date);

dtf = new Intl.DateTimeFormat('en-GB', { dateStyle: 'short', timeStyle: 'short'
}) dtf.resolvedOptions(); dtf.format(date)
...
// EX: new Intl.DateTimeFormat(['2-digit', '2-digit']).format(date) ->
// "20/12/2020"
...
vm::ExecutionStatus DateTimeFormat::initialize(
  ...
  const char *locale8 = conversion.getValue().c_str();
  impl_->locale8 = locale8; // store the UTF8 version of locale since it is used in almost all other functions
